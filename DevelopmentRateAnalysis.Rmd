...
We considered measurements obtained at the same temperature, of the same species, locality, and ontogenetic stage reported in the same publication to be redundant and we obtained the geometric mean, for analyses. They kept the number of the first set they include, with the prefix "c" for consolidated
...
4. Obtain Ropt (rate at Topt) and minTopt, maxTopt
```{r}
...
max_Topt_table: table of maximum temperatures that allow for development rate at maximum efficiency
min_Topt_table: table of minimum temperatures that allow for development rate at maximum efficiency. There are two tables because in some cases there are several optima instead of one. plots for these tables are almost the same, so we should choose 1 (min_Topt_table) to focus but analyze both and report if results were the same or different
...
Larva with TDopt at ~ 15C is Ephestia calidella

```{r}
ggplot(filter(max_Topt_table, quality != "inferred"), aes(x = abs(lat), y = maxTDopt))+
  geom_point()+
  theme_cowplot()+
  ylab("TDopt")+
...
```{r}
library(effects)
plot(Effect(c("absLat","lifestage"),fit1))
```
