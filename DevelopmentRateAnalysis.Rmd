...
Interval_table <- inner_join(Interval, add_info, by = "set")
Interval_table$lifestage <- factor(Interval_table$lifestage)
```
...
summary <-Interval_table %>% 
  group_by(lifestage) %>% 
  summarise(median = median(Ilength, na.rm = T)) 
  #quantile((Ilength,0.25, na.rm = T))#, c(0.25, 0.5, 0.75), type = 1,na.rm = T))
```
 
 
 Median length of the favorable interval (allows for development to occur at 50% efficiency) was `r `
