...
rm(comlete_sets) # list of sets thatare complete
...
```{r}
ggplot(max_Topt_table, aes(x = lifestage, y = maxTDopt))+
  geom_boxplot()+
  theme_cowplot()+
  ylab("TDopt (max)")+
...
```{r}
ggplot(max_Topt_table, aes(x = abs(lat), y = maxTDopt), col = "quality")+
  geom_point()+
  theme_cowplot()+
  ylab("TDopt (max)")+
  #geom_jitter(mapping = aes(x = lifestage, y = maxTDopt, col = family))+
...
#geom_jitter(mapping = aes(x = lifestage, y = minTDopt, col = family))
  ...
  ylim(10,40)
```
Minimum temperature that allows for optimum development rate
```{r}
...
```
This is to check outliers (sets 586, 1005, 175). 586 is fine, 1005 is incomplete, 175 is fine.
```{r}
ggplot(filter(Ana_dr, set == 175), aes(x = temp, y = dr))+
  geom_point()
...
ggplot(Interval_table, aes(x = abs(lat), y = Ilength), col = "quality")+
  geom_point()+
  theme_cowplot()+
  ylab("Ilength")+
  #geom_jitter(mapping = aes(x = lifestage, y = maxTDopt, col = family))+
...
library(lme4)
library(car)
hist(max_Topt_table$maxTDopt)
shapiro.test((max_Topt_table$maxTDopt))
fit1 <- lmer(maxTDopt ~ lifestage + (1|sp) + (1|family), data = filter(max_Topt_table, lifestage != "eggtoemergence")) # 
summary(fit1)
