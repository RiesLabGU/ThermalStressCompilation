...
=======
5. Calculate Ropt (maximum development rate) and Topt (temperature that allows for Topt)
>>>>>>> 8981efe0f8f45626984c40d16321be596f23b557
...
=======
We considered measurements obtained at the same temperature, of the same species, locality, and ontogenetic stage that were also reported in the same publication, to be redundant and we obtained the geometric mean, for analyses. They kept the number of the first set they include, with the prefix "c" for consolidated.
Redundant sets occur beacuse researchers varied experimental conditions such as: host plant, humidity and daylength, or because data were reported for males and females separately
>>>>>>> 8981efe0f8f45626984c40d16321be596f23b557
...
=======
There were `r length(unique(singles$set))` sets with a single optimum temperature and `r length(unique(multiple$set))` sets for which several temperatures allowed for development at optimum rates. I extracted both the minimum and maximum temperatures that allow for optimum development rate. Patterns are very simmilar.
>>>>>>> 8981efe0f8f45626984c40d16321be596f23b557
...
```
<<<<<<< HEAD
Lenght of the temperature interval that allows for development rates at >=50% efficincy.  
=======
The interval that allows for 
...
ggplot(filter(Interval_table, quality != "inferred"), aes(x = abs(lat), y = Ilength, col = family))+
  geom_point()+
  theme_cowplot()+
  ylab("D-interval length")+
  #geom_jitter(mapping = aes(x = lifestage, y = maxTDopt, col = family))+
...
TDopt. For analysis I discarded "eggtoemergence" because it integrates patterns of eggs, larvae and pupae. I also discarded sets in which the locality was "inferred" instead of reported by authors.
...
ggplot(pred, aes(x = x, y = predicted, colour = group, fill = group)) +
  ...
  geom_point(data = ToptAnalysis, mapping = aes(x = abs(lat), y = maxTDopt, col =group ))+
...
```{r}
#Imin
IntervalAnalysis <- filter(Interval_table, lifestage != "eggtoemergence", quality!= "inferred")
IntervalAnalysis$absLat <- abs(IntervalAnalysis$lat)


fit2 <- lmer(Imin ~ lifestage + absLat + (1|sp) + (1|family), data = IntervalAnalysis) # 
summary(fit2)
Anova(fit2) # 
plot(fit2) #
```
...
IntervalAnalysis$group <- IntervalAnalysis$lifestage
plot(pred2)
ggplot(pred, aes(x = x, y = predicted, colour = group, fill = group)) +
  geom_line()+
   geom_ribbon(aes(ymin = conf.low, ymax = conf.high, col = group), alpha = 0.1)+
  ...
  xlab("Absolute latitude")+
 # ylim(5, 20)+
  geom_point(data = IntervalAnalysis, mapping = aes(x = abs(lat), y = Imin, col =group ), alpha = 0.4)+
  theme_cowplot()
...
fit3 <- lmer(Imax ~ lifestage + absLat + (1|sp) + (1|family), data = IntervalAnalysis) # 
summary(fit3)
Anova(fit3) # 
plot(fit3) #
```

The maximum temperature did not vary across latitudes, or lifestages.

```{r}
ggplot(IntervalAnalysis, aes(x = absLat, y = Imax, col = lifestage))+
 geom_point()+
  theme_cowplot()+
  ylab("D-interval max")+
...
>>>>>>> 8981efe0f8f45626984c40d16321be596f23b557
fit4 <- lmer(Ilength ~ lifestage + absLat + (1|sp) + (1|family), data = IntervalAnalysis) # 
summary(fit4)
Anova(fit4) # 
plot(fit4) #
```
...
ggplot(IntervalAnalysis, aes(x = absLat, y = Ilength, col = lifestage))+
 geom_point()+
  theme_cowplot()+
  ylab("D-interval length")+
  scale_color_viridis_d(alpha = 0.5)+
...
```
<<<<<<< HEAD
Length of the interval of temperatures that allowed for development at >=50% efficiency. interval length varied from min `r min(IntervalAnalysis$Ilength, na.rm = T)`  to max `r max(IntervalAnalysis$Ilength, na.rm = T)`, mean = `r mean(IntervalAnalysis$Ilength, na.rm = T)`, SD = `r sd(IntervalAnalysis$Ilength, na.rm = T)`
=======

>>>>>>> 8981efe0f8f45626984c40d16321be596f23b557

```{r}
Intervar <- select(IntervalAnalysis, set, Ilength, Imin, Imax)
