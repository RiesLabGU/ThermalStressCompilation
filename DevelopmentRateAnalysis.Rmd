...
# Model Topt ~ life stage & absolute latitude

## 1. Rescaling variables
Transform continuous variables to standard deviation units
```{r}
...
ToptAnalysis$Topt_s <- scale(ToptAnalysis$maxTDopt)[,1]
ToptAnalysis$absLat_s <- scale(ToptAnalysis$absLat)[,1]

```
Topt ranges from `r min(ToptAnalysis$maxTDopt)` to `r max(ToptAnalysis$maxTDopt)`, 
...
ToptAnalysis <- filter(Topt_table, lifestage != "eggtoemergence", quality!= "inferred")
ToptAnalysis$absLat <- abs(ToptAnalysis$lat)
ToptAnalysis$lifestage <- factor(ToptAnalysis$lifestage, levels = c("pupa","egg", "larva"))

fit1_scaled <- lmer(Topt_s ~ lifestage + absLat_s + (1|sp), data = ToptAnalysis) #
fitlinear <- lm(Topt_s ~ lifestage + absLat_s, data = ToptAnalysis)
summary(fit1_scaled)
summary(fitlinear)
anova(fitlinear)
Anova(fitlinear)
...
# library(effectsize)
# fit1EffectSize <- effectsize(fit1)



dw_plot(fit1_scaled)+geom_vline(xintercept=0,lty=2)
```


Troubleshooting plot.design()
```{r}
pl <- select(ToptAnalysis, Topt_s, lifestage, absLat_s)
plot.design(Topt_s~ lifestage+ absLat_s, data = pl)
```


```{r}

ToptAnalysis$absLat_scaled <- abs(ToptAnalysis$lat)*100


fit1i <- lmer(maxTDopt ~ lifestage * absLat + (1|sp), data = ToptAnalysis) #
fit1 <- lmer(maxTDopt ~ lifestage +  absLat + (1|sp), data = ToptAnalysis) #
fit1_e <- lmer(maxTDopt ~ lifestage * absLat_scaled + (1|sp), data = ToptAnalysis) #
summary(fit1_e)
summary(fit1i)
summary(fit1)
Anova(fit1)
...
dw_plot(fit1)+geom_vline(xintercept=0,lty=2)
plot.design(fit1_scaled)
dw_plot(fit1_scaled)+geom_vline(xintercept=0,lty=2)
plot.design(fit1_scaled)
library(sjPlot)
library(sjmisc)
...
plot_model(fit1_scaled, y.offset = .4)
plot_model(fit1_scaled,
         facet.grid = FALSE,
         sort.est = "sort.all",
         y.offset = .4)
plot_model(fit1_scaled, type = "eff")
plot_model(fit1_scaled, type = "int")
