...
Lifestage and absolute latitude explained variation in maxSopt
...
ggplot(pred, aes(x = x, y = predicted, colour = group, fill = group)) +
  ...
  geom_point(data = SoptsAnalysis, mapping = aes(x = abs(lat), y = maxSopt, col =group ))+
...
Lifestage and absolute latitude explained variation in minSopt
...
ggplot(pred, aes(x = x, y = predicted, colour = group, fill = group)) +
  geom_line()+
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high, col = group), alpha = 0.1)+
  geom_point(data = SoptsAnalysis, mapping = aes(x = abs(lat), y = minSopt, col =group ))+
  scale_color_viridis_d()+
  scale_fill_viridis_d()+
  ylab("minSopt")+
  xlab("Absolute latitude")+
...
hist(IntervalAnalysis$Imin)
shapiro.test(IntervalAnalysis$Imin)
fit3 <- lmer(Imin ~ lifestage + absLat + (1|sp) + (1|family), data = IntervalAnalysis) #
summary(fit3)
Anova(fit3) # 
plot(fit3) #
```
Absolute latitude, but not lifestage affected Imin
...
fit4 <- lmer(Imax ~ lifestage + absLat + (1|sp) + (1|family), data = IntervalAnalysis) # 
summary(fit4)
Anova(fit4) # 
plot(fit4) #
```

Absolute latitude, but not lifestage affected variation in Imin
```{r}
...
ggplot(pred, aes(x = x, y = predicted, colour = group, fill = group)) +
  geom_line()+
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high, col = group), alpha = 0.1)+
  geom_point(data = IntervalAnalysis, mapping = aes(x = abs(lat), y = Imax, col =group ))+
  scale_color_viridis_d()+
  scale_fill_viridis_d()+
  ylab("Imax")+
  xlab("Absolute latitude")+
...
fit5 <- lmer(Ilength ~ lifestage + absLat + (1|sp) + (1|family), data = IntervalAnalysis)# 
summary(fit5)
Anova(fit5) # 
plot(fit5) #
```
...
ggplot(pred, aes(x = x, y = predicted, colour = group, fill = group)) +
  #geom_line()+
  #geom_ribbon(aes(ymin = conf.low, ymax = conf.high, col = group), alpha = 0.1)+
  geom_point(data = IntervalAnalysis, mapping = aes(x = abs(lat), y = Ilength, col =group ))+
  scale_color_viridis_d()+
  scale_fill_viridis_d()+
  ylab("Ilength")+
  xlab("Absolute latitude")+
...
Length of the interval of temperatures that allowed for >=50% survival. interval length varied from min `r min(IntervalAnalysis$Ilength, na.rm = T)`  to max `r max(IntervalAnalysis$Ilength, na.rm = T)`, mean = `r mean(IntervalAnalysis$Ilength, na.rm = T)`, SD = `r sd(IntervalAnalysis$Ilength, na.rm = T)`
