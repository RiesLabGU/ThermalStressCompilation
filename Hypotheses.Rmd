...
A1 <- ggplot(ToyData, aes(x = temp, y = p1))+
  ...
  scale_y_continuous(name = "Development rate", sec.axis = sec_axis(~ .*1, name = "Survival",breaks = c(0, 0.5, 1)),breaks = c(0, 0.5, 1))+
  theme_cowplot()
...
B1 <- ggplot(ToyData, aes(x = temp, y = p4))+
 ...
 # geom_segment(x = 20.5, y = 0, xend = 34.2, yend = 0, linetype = 1, col = "grey40")+
  annotate("text", x = 20.5, y = 0, label = expression("S"[l]))+
  annotate("text", x = 34.2, y = 0, label = expression("S"[h]))+
  #geom_segment(x = 9.5, y = 0.49, xend = 39, yend = 0.49, linetype = 2)+
  theme_cowplot()+
  #annotate("text", x =5, y = 1, label = "B")+
