...
initialplot <- ggplot(ToyData, aes(x = temp, y = p1))+
  ...
  geom_line(data = ToyData, mapping = aes(x = temp, y = dr2), size = 2, col = "black", linetype = 3)+ #development rate
 # performance
  geom_segment(x = 17.5, y = -0.08, xend = 31.3, yend = -0.08, linetype = 1, col = "grey", size = 2)+
  ...
  xlab("")+
  ylim(-0.25,1)+
  annotate("rect", xmin = 17.5, xmax = 31.3, ymin = 0, ymax = 1, fill = "grey", alpha = 0.3)+
  # annotate("text", x = 12, y = -0.08, label = "chill stress")+
  # annotate("text", x = 39.5, y = -0.08, label = "heat stress")+
  # annotate("text", x = 28, y = -0.08, label = "optimal range")+
...
heatdhigher <- ggplot(ToyData, aes(x = temp, y = p3))+
  geom_line(size = 2, col = "grey")+ # performance
  geom_line(data = ToyData, mapping = aes(x = temp, y = slower/100), size = 1, linetype = 1, col = "grey40")+ # survival
  geom_line(data = ToyData, mapping = aes(x = temp, y = dr3), size = 2, col = "black", linetype = 3)+ #development rate
 # performance
  geom_segment(x = 20.5, y = -0.08, xend = 31.8, yend = -0.08, linetype = 1, col = "grey", size = 2)+
  annotate("text", x = 19.5, y = -0.08, label = expression("P"[low]))+
  annotate("text", x = 33, y = -0.08, label = expression("P"[high]))+
  ...
  xlab("")+
  ylim(-0.25,1)+
  annotate("rect", xmin = 20.5, xmax = 31.8, ymin = 0, ymax = 1, fill = "grey", alpha = 0.3)#+
  # annotate("text", x = 12, y = -0.08, label = "chill stress")+
  # annotate("text", x = 39.5, y = -0.08, label = "heat stress")+
  # annotate("text", x = 28, y = -0.08, label = "optimal range")+
  # 
...
```
For heat-tolerant populations, if Dhigh is larger than Shigh, then performance is limited by x

```{r}
heatdhigher
```

For heat-tolerant populations, if Dhigh is smaller than Shigh, then performance is limited by y.
```{r}
initialplot
```
...
heatdlower<- ggplot(ToyData, aes(x = temp, y = p4))+
  geom_line(size = 2, col = "grey")+ # performance
...
```{r}
ToyData$p5 <- ToyData$dr4 *(ToyData$surhigher/100)
heatdlower<- ggplot(ToyData, aes(x = temp, y = p5))+
  geom_line(size = 2, col = "grey")+ # performance
