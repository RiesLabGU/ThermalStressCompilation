...
## Anna comment - I am having issues with unnest() creating a heinous dataframe with 107/108 columns. I'm not sure how to fix it because it appears the syntax for unnest() is different now. 


## Subset the data by lifestage
...
```
 Each line corresponds to one temperature treatment
```{r}
ggplot(filter(ontosurvival, id == 80), aes(x = stagenum, y = survival, col = factor(round(temp))))+
  geom_point()+
  geom_line(aes( y = survival, x = stagenum, col = factor(round(temp))))+
  #theme(legend.position = "none")+
  xlab("Life stage 1:egg, 2:larva, 3:pupa")+
