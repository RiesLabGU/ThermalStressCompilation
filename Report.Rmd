...
ggplot(inter, aes(x = temp))+
  geom_bar()+Show in New WindowClear OutputExpand/Collapse Output
 [1] "pest"            "migratory"       "forestpest"      "biocontrol"      "pest/biocontrol" "silk"               
 [7] "ornamental"      "insectivorous"   "reseach"         "wild"            "parasitoid"      "nopest"         
[13] "tea"             "endangered"     
...
Try `df %>% unnest(c(quality_dr, quality_sur))`, with `mutate()` if needed
Show in New WindowClear OutputExpand/Collapse Output
...
Show in New WindowClear OutputExpand/Collapse Output
unnest() has a new interface. See ?unnest for details.
Try `df %>% unnest(c(data, s_dt))`, with `mutate()` if needed
Show in New WindowClear OutputExpand/Collapse Output
