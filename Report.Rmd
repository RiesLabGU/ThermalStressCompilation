...
Cols with relevant info to identify these groups are: sex, lifestage, id, temp, host. I'll work on this, It looks like the 
only species affected are Chilo auricilius and Metissa plana.

```{r}

combsurvival <- ontosurvival[, c("set", "id", "sp", "lifestage", "temp", "survival", "stagenum")]
head(ontosurvival)
# ontosurvival %>% mutate(key = factor(paste(id, sp, sep = "-")))
# unique(ontosurvival$sex)
# ontosurvival_small <- data.frame(ontosurvival[, c("key", "lifestage", "temp", "survival")])
#   ontosurvival_small <- ontosurvival_small %>% 
#     
#   unique(factor(inter$idaid))
#   ontosurvival_small %>% 
#     group_by(key) %>% 
