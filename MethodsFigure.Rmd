...
hypofit2 <- fit_dr(set_3)
hyposu <- fit_survival(set_944)
hyposu2 <- fit_survival(set_3)
...
d_opt2 <- max(set_3$dr)
dTopt2 <- filter(set_3, dr == d_opt2)[["temp"]]

t0table <- get_t0(set_3)

linear_3 <- filter(set_3, temp <= dTopt2, dr > 0)
model <- lm(dr~temp, data = linear_3)
linear_predicted <-  tibble(temp = c(1:40),
  predicted_dr = predict.lm(model, newdata = tibble(temp = c(1:40)), type = "response"))
t0 <- t0table[[1]]
ggplot(data = linear_predicted, aes(x = temp, y = predicted_dr)) + 
  geom_line() +
  geom_point(data = linear_3, mapping = aes(x = temp, y = dr))+
  geom_point(data = set_3, mapping = aes(x = temp, y = dr), shape = 1)+
   geom_text(x = 7.5, y = 0.02, label = "T0 = 9.7")+
...
interpolation <- as.data.frame(approx(x = set_3$temp, y = set_3$dr, xout = out, method = "linear", rule = 1))
names(interpolation) <- c("temp", "dr")
sub_min <- head(filter(interpolation, dr >= d_50), 1)[[1]]
sub_max <- tail(filter(interpolation, dr >= d_50), 1)[[1]]

ggplot(data = predicted2, aes(x = temp, y = dr)) + 
  #geom_line(linetype = 2) +
  #geom_point(data = linear_636, mapping = aes(x = temp, y = dr))+
  geom_point(data = set_3, mapping = aes(x = temp, y = dr))+
  geom_line(data = set_3, mapping = aes(x = temp, y = dr), alpha = 0.6)+
  
  #geom_abline(slope = t0table[[2]], intercept = t0table[[1]])+
  #geom_abline(slope = 0.01393435, intercept = 3.761635)+
  geom_segment(x = sub_min, xend = dTopt2, y = d_50, yend = d_50, linetype = 3)+
  geom_segment(x = dTopt2, xend = sub_max, y = d_50, yend = d_50, linetype = 2)+
  # geom_text(x = 3.761635, y = 0.03, label= "T0")+
  # geom_text(x = 3.761635, y = 0, label = "3.8")+
  geom_segment(x  = dTopt2, xend  = dTopt2, y = 0, yend = d_opt2, linetype = 2)+
  theme_cowplot()+
...
max_sTopt <- tail(filter(set_944, survival >=  50), 1) [[1]]
min_sTopt <- head(filter(set_944, survival >= 50), 1)[[1]]

out <- seq(from = 0, to = 40, by = 0.1)
interpolation <- as.data.frame(approx(x = set_944$temp, y = set_944$survival, xout = out, method = "linear", rule = 1))
names(interpolation) <- c("temp", "survival")
sub_min <- head(filter(interpolation, survival >= 50), 1)[[1]]
sub_max <- tail(filter(interpolation, survival >= 50), 1)[[1]]

ggplot(data = predicted2, aes(x = temp, y = dr)) + 
  #geom_line(linetype = 2) +
  #geom_point(data = linear_636, mapping = aes(x = temp, y = dr))+
  geom_point(data = set_944, mapping = aes(x = temp, y = survival/100))+
  geom_line(data = set_944, mapping = aes(x = temp, y = survival/100), alpha = 0.6)+
  
  #geom_abline(slope = t0table[[2]], intercept = t0table[[1]])+
  #geom_abline(slope = 0.01393435, intercept = 3.761635)+
  geom_segment(x = sub_min, xend = sub_max, y = 0.5, yend = 0.5, linetype = 3)+
