...
anova(Dopt_i, Dopt_a, Dopt_r, test.statistic = "Chisq") # keep additive model
...
summary(Dopt_a_ML)
```
According to this mode, Dopt was close to 32. for all lifestages and Dopt increased 0.6 degrees C for every 10 degrees in latitude. Variation among species: 2.3
...
# RUn this to asses the effect of phylogeny on Popt
...
```{r}
# pglmm.predicted.values(Dopt_phylo, gaussian.pred = "nearest_node")
# communityPGLMM.predicted.values(Dopt_phylo, gaussian.pred = "nearest_node")
# pglmm.predicted.values(Dopt_phylo, gaussian.pred = "tip_rm")[[1]]
# 
# fitted(Dopt_phylo)
# confint(Dopt_phylo)
# effects(Dopt_nophylo)
# 
# predict(Dopt_phylo, interval = "confidence")
# fitted.communityPGLMM(Dopt_phylo)
# resid(Dopt_phylo)
summary(Dopt_phylo)
```
...
```{r}
pglmm.predicted.values(Dopt_phylo, gaussian.pred = "tip_rm")
```

```{r}
#pglmm.plot.ranef(x = Dopt_phylo, site.var = "lifestage")
#pglmm.plot.ranef(x = Dopt_phylo, site.var = "absLat")
#plot_model(Dopt_phylo, type = "pred", terms = c("lifestage", "absLat"))+
 # theme_cowplot()+
  #scale_color_viridis_d()
```
...
anova(Dupper_i, Dupper_a, Dupper_r, test.statistic = "Chisq") # keep additive model
...
# 4. Dlow (Dlower)
Dhigh ranged from `r min(D.set$Dlower, na.rm = T)` to `r max(D.set$Dlower, na.rm = T)`, mean =  `r mean(D.set$Dlower, na.rm = T)`, SD = `r sd(D.set$Dlower, na.rm = T)`
Latitude ranged from `r min(D.set$absLat, na.rm = T)` to `r max(D.set$absLat, na.rm = T)`
