...
plot_model(fit1_randomMLs, type = "pred", terms = c("lifestage", "absLat"))+
  theme_cowplot()+
  scale_color_viridis_d()
```
...
fit2_fullrandom <- lmer(Dwidth ~ lifestage + absLat + lifestage:absLat + (lifestage|sp), data = D.set) #
...
## no significant difference use full model

# the effect of lifestage within species was very small, fit another model just with species:
fit2_randomML <- lmer(Dwidth ~ factor(lifestage) + absLat + lifestage:absLat + (1|sp), REML = F, data = as.data.frame(D.set))
...
# Plot model predictions
# type = c("est", "re", "eff", "pred", "int", "std", "std2", "slope", "resid", "diag")
plot_model(fit2_randomML, type = "int")+
  theme_cowplot()+
  scale_color_viridis_d()
