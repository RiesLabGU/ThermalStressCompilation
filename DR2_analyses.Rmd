...
fit1_full <- lmer(Dopt_s ~ -1 + lifestage + absLat + lifestage:absLat + (1|sp), data = Dopt_table) #
...
plot.design(Dopt_s ~ -1 + lifestage * absLat + sp , data = Dopt_table)
plot.design(Dopt_s ~ -1 + lifestage * absLat , data = Dopt_table)
# this does not work:
...
plot_model(fit1_full, type = "pred") # predicted values (marginal effects)
plot_model(fit1_full, type = "re") # random effects
plot_model(fit1_full, type = "emm")# look up
