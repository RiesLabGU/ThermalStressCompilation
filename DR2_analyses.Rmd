...
2. Models with taxonomic correction (nested random effect)
3. Models with lifestage within species (lifestage|sp) as a random effect
4. Models with species (1|sp) as a random effect
5. Correlation coefficients of development variables
6. PCA
...
# 2. Models with taxonomic correction

```{r}

# Models with taxonomy as a nested random effect
...
# Fit interaction, aditive and simple models with ML
full.nested <- lme(Dopt ~ lifestage + absLat + lifestage:absLat, random = ~ 1|family/genus/species, data = D.set, method = 'ML')
a.nested <- lme(Dopt ~ lifestage + absLat, random = ~ 1|family/genus/species, data = D.set,  method = 'ML')

anova.lme(full.nested,a.nested, test = "TRUE") # Keep aditive model
```
We kept the aditive model

```{r}
# fit better model using REML
nested <- lme(Dopt ~ -1 + lifestage + absLat, random = ~ 1|family/genus/species, data = D.set, method = "REML")

nested
```

```{r}
nested$coefficients$fixed
tidy(nested)
```

Dopt decreased 0.7 degrees with every 10 degrees of latitude


```{r}

D.set$group <- factor(D.set$lifestage)
pred <- ggpredict(nested, terms = c("absLat","lifestage"))
...
# 4. Models with lifestage within species (lifestage|sp) as a random effect
...
# With taxonomy as a nested rando effect
full.nested <- lme(Dopt ~ lifestage + absLat + lifestage:absLat, random = ~ 1|family/genus/species, data = D.set, method = 'ML')
a.nested <- lme(Dopt ~ lifestage + absLat, random = ~ 1|family/genus/species, data = D.set,  method = 'ML')
lat.nested <- lme(Dopt ~ absLat, random = ~ 1|family/genus/species, data = D.set, method = 'ML')
life.nested <- lme(Dopt ~ lifestage, random = ~ 1|family/genus/species, data = D.set,  method = 'ML')
anova(full.nested, a.nested, lat.nested, life.nested)
anova(full.nested,a.nested)
# Keep aditive model:
aic.a.nested <- AIC(a.nested)
# fit better model using REML
nested <- lme(Dopt ~ -1 + lifestage + absLat, random = ~ 1|family/genus/species, data = D.set, method = "REML")
summary(nested)
nested
...
plot_model(nested, type = "pred", terms = c("lifestage", "absLat"))+
  theme_cowplot()+
  scale_color_viridis_d()
```
