...
slatlifeI <- lmer(sDopt ~ -1 + sabsLat + lifestage + (1|sp), data = as.data.frame(D.set)) #
...
# fit better model with ML
fit1_randomML <- lmer(Dopt ~ lifestage + absLat + (1|sp), REML = F, data = as.data.frame(D.set))
...
Do same model on continuous variables converted to sd units to faciliate comparison of effect sizes
```{r}
D.set <- D.set %>% 
  mutate(sDopt = scale(Dopt), 
         sabsLat = scale(absLat))

Scalefit1_randomML <- lmer(sDopt ~ lifestage + sabsLat + (1|sp), REML = F, data = as.data.frame(D.set))
sfull <- lmer(sDopt ~ lifestage + sabsLat + lifestage:sabsLat + (1|sp), data = as.data.frame(D.set)) #
slatlife <- lmer(sDopt ~ sabsLat + lifestage + (1|sp), data = as.data.frame(D.set)) #
slat <- lmer(sDopt ~ sabsLat + (1|sp), data = as.data.frame(D.set)) #
slife <- lmer(sDopt ~ lifestage + (1|sp), data = as.data.frame(D.set)) #
anova(sfull, slatlife, slat, slife, test.statistic = "Chisq")
summary(slatlife)
summary(Scalefit1_randomML)
Anova(fit1_randomML)
D.set <- as.data.frame(D.set)

library(lme4)
example(glmer)
coef(summary(Scalefit1_randomML))
coef(summary(Scalefit1_randomML))[,"z value"]
...
```{r}
plot_model(fit1_randomML, type = "pred", terms = c("absLat", "lifestage"))+
  theme_cowplot()+
  geom_point(data = fit1_randomML, mapping = aes(x = absLat, y = Dopt, col = group))+
  scale_color_viridis_d()
```
