...
surv_range <- function(table){
  ...
  #5  
    new_row <- c(setvals[i, 1], min_interpol, max_interpol, mid)
    surv_range_vals <- rbind(surv_range_vals, new_row)
  } 
  setNames(surv_range_vals, c("set", "minTemp", "maxTemp", "survMid"))
  print(surv_range_vals)
  save(surv_range_vals, file = "surv_range_vals.csv")
}
...
```{r}
surv_range_vals <- read.csv("surv_range_vals.csv")
```


### Space for troubleshooting:

#### Checking out one of these NA sets - FIXED

```{r}
set_179 <- filter(inter, set == 179)

set_179 <- set_179[complete.cases(set_179$survival), ]

max_179 <- max(set_179$survival)
min_179 <- min(set_179$survival)

```

#### Checking how it's pulling out sets - CORRECT

```{r}
settest <- filter(inter, set == setvals[4, 1])
    settest <- settest[complete.cases(settest$survival), ]
```
