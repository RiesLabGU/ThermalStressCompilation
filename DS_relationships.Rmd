...
The higher limit for development Dhigh ranged from `r min(res$Dupper, na.rm = T)` to `r max(res$Dupper, na.rm = T)`  Shigh which in turn ranged from `r min(res$Supper, na.rm = T)` to `r max(res$Supper, na.rm = T)`
...
Dlow for eggs, larvae and pupae ranged from `r min(res$low, na.rm = T)` to `r max(res$Dlow, na.rm = T)`, with a mean (+_SD) of `r min(res$Dopt, na.rm = T)` +- `r SD(res$Dopt, na.rm = T)`
...
lows <- ggplot(res_plot5, aes(x = Slower, y = Dlower))+
  geom_point()+
  geom_point(data = res_plot2, aes(x = Supper, y = Dupper), shape = 1)+
  geom_point(data = res_plot3, aes(x = Swidth, y = Dwidth), shape = 3)+
  ylim(0,40)+
  xlim(0,40)+
